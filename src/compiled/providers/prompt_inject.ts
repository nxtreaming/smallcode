...
import type { ChatRequest, ChatResponse, IModelProvider } from "./types";
import { approxTokens } from "./types";
declare const require: (id: string) => any;

export type InjectionPosition = "prepend" | "append" | "replace";
...
export default class PromptInjectProvider implements IModelProvider {
  readonly name = "prompt_inject";
  private innerName: string;
  private innerInstance: IModelProvider | null = null;
  private injections: PromptInjection[];

  constructor(options: PromptInjectOptions) {
    this.innerName = options.inner;
    this.injections = options.injections || [];
  }

  private getInner(): IModelProvider {
